  ...
  return (
    <div className="pt-20 pb-32 max-w-[1440px] mx-auto px-5 sm:px-8 md:px-12 text-ink font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-10 items-stretch">
        ...
        <div className="lg:col-span-6 border border-line bg-[#FAF9F5] p-8 sm:p-10 flex flex-col justify-between hover:border-grey-3 transition-colors duration-200">
          <div>
            {/* Top Badge Line */}
            <div className="flex items-center justify-between text-xs pb-4 border-b border-line text-grey-7">
              ...
              <span className="text-[11px] tracking-wider uppercase">KOREA · 2026</span>
            </div>
            ...
            {/* Bullet Points */}
            <div className="mt-8 space-y-3.5 text-xs sm:text-sm text-ink">
        ...
        <div className="lg:col-span-6 border border-line bg-[#FAF9F5] p-8 sm:p-10 flex flex-col justify-between hover:border-grey-3 transition-colors duration-200">
          <div>
            ...
            <div className="flex items-center justify-between text-xs pb-4 border-b border-line text-grey-7">
              <span>CREDITS</span>
              <span className="tracking-wider uppercase text-[11px]">SYSTEM STACK</span>
            </div>
            ...
            <div className="space-y-3">
              {techStack.map((item) => (
                <div
                  key={item.name}
                  className="p-4 bg-paper border border-line flex flex-col justify-between gap-1.5 hover:border-grey-3 transition-all duration-150 group"
                >
                  <div className="flex items-baseline justify-between gap-2 text-xs">
                    <span className="text-sm font-medium text-ink group-hover:text-signal transition-colors">{item.name}</span>
                    <span className="text-grey-7 text-[11px] shrink-0 font-normal">{item.role}</span>
                  </div>
